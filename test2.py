...
class CullingWidget(QWidget):
    ...
    def toggle_select(self):
        ...
        self._post_task(10, write_xmp_sidecar, p.path, {'selected': p.selected})
    ...
    def set_rating(self, rating: int):
        ...
        # If current rating is the same, set it to 0 (toggle off)
        new_rating = 0 if p.rating == rating else rating
        p.rating = new_rating
        self._post_task(10, write_xmp_sidecar, p.path, {'rating': p.rating})
    ...
    def set_color_label(self, label: Optional[str]):
        p = self._current()
        if not p: return
        # If current label is the same, clear it (toggle off)
        new_label = "" if p.color_label == label else (label or "")
        p.color_label = new_label
        self._post_task(10, write_xmp_sidecar, p.path, {'color_label': p.color_label})
